...
void GetAddresses()
{
	...
	Global::m_UWorld = reinterpret_cast<Classes::UWorld**>(AddrUWorld + 7 + OffUWorld);
	...
	Classes::UObject::GObjects = reinterpret_cast<Classes::FUObjectArray*>(AddrGObj + 7 + OffGObj);
	...
	Classes::FName::GNames = *reinterpret_cast<Classes::TNameEntryArray**>(AddrGName + 7 + OffGName);
	...
	if (Global::m_PersistentLevel != nullptr && Global::m_LocalPlayer != nullptr && Global::m_LocalPlayer->PlayerController->AcknowledgedPawn != nullptr)
	{
		...
		for (int i = 0; i < actors.Num(); i++)
		{
			...
			if (actor->IsA(Classes::ATASC_Enforcer_C::StaticClass()))
			{
				...
				// Draw Box
				Classes::FVector HeadPos, LeftFootPos, RightFootPos, BottomPos;
...
HRESULT __stdcall hookD3D11Present(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
{
	...
	renderer->begin();
	renderer->drawText(Vec2(14.0f, 10.0f), L"Made by ImShotZz", RED_COLOR, 0, 12.0f, FONT_TYPE);
